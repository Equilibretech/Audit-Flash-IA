...
function downloadPDF() {
    ...
    if (typeof pdfMake === 'undefined') {
        ...
        Promise.all([
            ...
            console.error('Erreur chargement PDFMake:', error);
            button.textContent = originalText;
            button.disabled = false;
...
function generateExamplePDF() {
    try {
        ...
        // Vérifier si PDFMake est disponible
        if (typeof pdfMake === 'undefined') {
            alert('PDFMake n\'est pas encore chargé. Veuillez attendre quelques secondes et réessayer.');
            return;
        }
        ...
        setTimeout(() => {
            alert('✅ PDF d\'exemple téléchargé ! Pour obtenir une roadmap personnalisée, remplissez le formulaire et cliquez sur "Générer ma roadmap IA".');
        }, 500);
        
    } catch (error) {
        console.error('Erreur génération PDF exemple:', error);
        alert('Erreur lors de la génération du PDF d\'exemple: ' + error.message);
    }
}
